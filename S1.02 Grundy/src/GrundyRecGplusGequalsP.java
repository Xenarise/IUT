...
/**
 * Grundy game with AI for the machine (VERSION 4)
 * This program is version 4 of the Grundy game.
...
class GrundyRecGplusGequalsP {
	...
	/**
	 * An array representing the types of heaps. Each index corresponds to a heap, 
	 * and the value at that index indicates the type of the heap.
	 */
	...
	/**
	 * Returns a sorted list of losing heaps from the given game state.
	 * 
	 * @param jeu - A dynamic list representing the game state.
	 * @return A new sorted list containing only heaps that are not winning.
	 */
    ArrayList<Integer> TasPerdant(ArrayList<Integer> jeu){
		ArrayList<Integer> jeuMethode = new ArrayList<Integer>(); // la methode 3.4 
		for(int i = 0; i < jeu.size(); i++){
			...
			tas.sort(null); // Tri croissant des tas
					
			// Ajouter à la méthode uniquement les tas perdants
			if(!estConnuePerdante(tas)){
				jeuMethode.add(jeu.get(i));
			}
		}
		jeuMethode.sort(null); // tris jeu methode dans l'ordres croissant
		
		return jeuMethode;
	}
	
	/**
	 * Tests the TasPerdant method with visual output.
	 */
	void testTasPerdant(){
		//test Visuel
		System.out.println("***** testTasPerdant ********");
		posPerdantes.clear();
		posPerdantes.add(new ArrayList<>(Arrays.asList(4, 20)));
		ArrayList<Integer> jeu1 = new ArrayList<>(Arrays.asList(3, 4, 20));
		ArrayList<Integer> resultat1 = TasPerdant(jeu1);
    ...
    boolean estConnuePerdante ( ArrayList<Integer> jeu ){
		...
		// normalisation est comme le ArrayList jeu sans les 2 et 1
		for ( int i = 0; i < jeu.size(); i++ ){
			if ( jeu.get(i)>2 ) {
				normalisation.add( jeu.get(i) );
			}
		}
		normalisation.sort(null); //tris l'ArrayList en ordre croissant
	...
	/**
	 * Checks if two heaps belong to the same type using the global `type` array.
	 * 
	 * @param tas1 - The index of the first heap.
	 * @param tas2 - The index of the second heap.
	 * @return true if both heaps have the same type in the global `type` array; false otherwise.
	 */
	boolean memeType(int tas1, int tas2) {
		return type[tas1] == type[tas2];
	}
	
	/**
	 * Simplifies the list of heaps by removing pairs of winning heaps of the same type.
	 * 
	 * @param jeu - The dynamic list representing the game state.
	 */
	void simplifierCouplesGagnants(ArrayList<Integer> jeu) {
		for (int i = 0; i < jeu.size(); i++) {
			for (int j = i + 1; j < jeu.size(); j++) {
				int tas1 = jeu.get(i);
				int tas2 = jeu.get(j);

				if (memeType(tas1, tas2)) {
					// Tas de même type : G + G = P
					if (estConnueGagnante(new ArrayList<>(Arrays.asList(tas1))) &&
						estConnueGagnante(new ArrayList<>(Arrays.asList(tas2)))) {
						jeu.remove(j);
						jeu.remove(i);
						j--;
					}
    ...
    boolean estPerdante(ArrayList<Integer> jeu) {
		// Simplifier les couples gagnants avant de déterminer si le jeu est perdant
		simplifierCouplesGagnants(jeu);
		...
		if (estConnueGagnante(jeu)){
			...
			if (jeu == null) {
				...
				if ( !estPossible(jeu) ) {
					...
					ArrayList<Integer> jeuTasPerdant = TasPerdant(jeu);
					
					// Implémentation de la règle : G + P = G et P + P = P
					boolean tasPerdant = estConnuePerdante(jeuTasPerdant);
					boolean tasGagnant = estConnueGagnante(jeuTasPerdant);
					
					if(tasPerdant == true){
						ret = true;
					}else if(tasGagnant == true){
						ret = false;
					}else if(tasPerdant == false && tasGagnant == false){
						...
						if (ret){
							...
							perdant.sort(null); // tris le tableau
							...
							gagnant.sort(null); // tris le tableau
