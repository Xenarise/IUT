...
/**
 * Grundy game with AI for the machine (VERSION 1)
 * This program is version 1 of the Grundy game.
...
class GrundyRecPerdantes {
	 ...
	 */
	long cpt = 0; // variable qui est utiliser pour compter le nb tout de estGagnante
    ...
    boolean estConnuePerdante ( ArrayList<Integer> jeu ){
		...
		normalisation.sort(null); // trie le tableau normalisation par ordre croissant
    ...
    boolean estPerdante(ArrayList<Integer> jeu) {
		...
		if(estConnuePerdante(jeu)){ // si estConnuePerdantes est true on ne fait aucun calcul
			ret = true;
		}else{
